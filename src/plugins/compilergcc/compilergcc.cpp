 ...
 * $HeadURL: https://svn.code.sf.net/p/codeblocks/code/branches/release-25.03/src/plugins/compilergcc/compilergcc.cpp $
 */
...
//    Manager::Get()->GetLogManager()->DebugLogError(currentPath);
    ...
    // OPTIMIZATION FIX: If compiler options haven't been loaded yet, skip validation
    if (cApp.IsEmpty())
    {
        Manager::Get()->GetLogManager()->DebugLog(wxString::Format("Compiler '%s' C program not set, skipping environment setup", compiler->GetName()));
        return;
    }
...
//    Manager::Get()->GetMessageManager()->Log(m_PageIndex, _T("BuildStateManagement")));
    ...
    if (compiler)
        compiler->EnsureOptionsLoaded(); // Lazy load compiler options if not yet loaded
    DirectCommands dc(this, compiler, m_pBuildingProject, m_PageIndex);
    dc.m_doYield = true;
