 ...
 * $HeadURL: https://svn.code.sf.net/p/codeblocks/code/branches/release-25.03/src/sdk/autodetectcompilers.cpp $
 */
...
AutoDetectCompilers::AutoDetectCompilers(wxWindow* parent)
{
    ...
    if (list)
    {
        ...
        for (size_t i = 0; i < count; ++i)
        {
            ...
            {
                // OPTIMIZATION: Skip auto-detection if compiler is already configured
                if (!path.empty() && wxFileName::DirExists(path_no_macros))
                {
                    // Compiler already configured and path exists - skip expensive detection
