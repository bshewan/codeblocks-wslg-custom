...
Compiler* CompilerFactory::CreateCompilerCopy(Compiler* compiler, const wxString& newName)
{
    ...
    // OPTIMIZATION: Skip ReloadOptions() at registration time
    // Options will be loaded lazily when compiler is first used
    // newC->ReloadOptions();
    RegisterCompiler(newC);
    // OPTIMIZATION: Skip LoadSettings() at registration time
    // User settings will be loaded lazily when compiler is first used
...
{
    // OPTIMIZATION: Skip loading settings for all compilers during initial factory load
    // Settings will be loaded on-demand when compiler is first used via EnsureOptionsLoaded()
    // However, load the default compiler immediately so debugger controls work
    
    if (s_DefaultCompiler)
    {
        s_DefaultCompiler->EnsureOptionsLoaded();
    }
