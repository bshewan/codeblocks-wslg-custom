 ...
 *
 * $Revision: 13627 $
 * $Id: compiler.cpp 13627 2025-03-02 18:17:10Z mortenmacfly $
 * $HeadURL: https://svn.code.sf.net/p/codeblocks/code/branches/release-25.03/src/sdk/compiler.cpp $
 */
...
void Compiler::Reset()
{
    ...
    for (int i = 0; i < ctCount; ++i)
        m_Commands[i].clear();
    // OPTIMIZATION: Mark options as not loaded - will load on-demand
    m_OptionsLoaded = false;
...
{
    // OPTIMIZATION: Lazy load options on first validity check
    EnsureOptionsLoaded();
...
void Compiler::EnsureOptionsLoaded()
{
    if (!m_OptionsLoaded)
    {
        LoadDefaultOptions(GetID());
        ...
        // Also load user settings (paths, programs, etc.)
        wxString baseKey = m_ParentID.IsEmpty() ? _T("/sets") : _T("/user_sets");
        LoadSettings(baseKey);
        
        // Fire settings changed event so debugger and other plugins update
        CodeBlocksEvent event(cbEVT_COMPILER_SETTINGS_CHANGED);
        event.SetString(GetID());
        event.SetInt(static_cast<int>(CompilerFactory::GetCompilerIndex(this)));
        event.SetClientData(static_cast<void*>(this));
